...
/* Get a pixel from an image, if outside image, black or nearest pixel */
...
double interpolate_2d_cubic_gray_black(double Tlocalx, double Tlocaly, int *Isize, double *Iin) {
    ...
    /* Determine of the zero neighbour */
    fTlocalx = floor(Tlocalx); fTlocaly = floor(Tlocaly);
    xBas0=(int) fTlocalx; yBas0=(int) fTlocaly;
    ...
    /* t vector multiplied with 4x4 bi cubic kernel gives the to q vectors */
...
void interpolate_forward_2d_double(double *Iin, double *Tlocalx, double *Tlocaly, int *Isize, int *ImageSize, double *Iout)
{
    ...
    for(jy=0; jy<Isize[1]; jy++)
    {
        for(jx=0; jx<Isize[0]; jx++)
        {
            ...
            b1=absd(x2-Tlocalx[jxmin+jy*Isize[0]]);
            b2=absd(x2-Tlocalx[jxmax+jy*Isize[0]]);
            b3=absd(y2-Tlocaly[jx+jymin*Isize[0]]);
            b4=absd(y2-Tlocaly[jx+jymax*Isize[0]]);
	...
	for(jz=0; jz<Isize[2]; jz++)
    {
		for(jy=0; jy<Isize[1]; jy++)
		{
			for(jx=0; jx<Isize[0]; jx++)
			{
				...
				b1=absd(x2-Tlocalx[jxmin+jy*Isize[0]+jz*sPixels]);
				b2=absd(x2-Tlocalx[jxmax+jy*Isize[0]+jz*sPixels]);
				b3=absd(y2-Tlocaly[jx+jymin*Isize[0]+jz*sPixels]);
				b4=absd(y2-Tlocaly[jx+jymax*Isize[0]+jz*sPixels]);
				b5=absd(y2-Tlocaly[jx+jy*Isize[0]+jzmin*sPixels]);
				b6=absd(y2-Tlocaly[jx+jy*Isize[0]+jzmax*sPixels]);
	...
	for(jz=0; jz<Isize[2]; jz++)
    {
		for(jy=0; jy<Isize[1]; jy++)
		{
			for(jx=0; jx<Isize[0]; jx++)
			{
				...
				b1=absf(x2-Tlocalx[jxmin+jy*Isize[0]+jz*sPixels]);
				b2=absf(x2-Tlocalx[jxmax+jy*Isize[0]+jz*sPixels]);
				b3=absf(y2-Tlocaly[jx+jymin*Isize[0]+jz*sPixels]);
				b4=absf(y2-Tlocaly[jx+jymax*Isize[0]+jz*sPixels]);
				b5=absf(y2-Tlocaly[jx+jy*Isize[0]+jzmin*sPixels]);
				b6=absf(y2-Tlocaly[jx+jy*Isize[0]+jzmax*sPixels]);
